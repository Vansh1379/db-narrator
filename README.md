...
- **SQL upload & session bootstrapping** – Drag-and-drop `.sql` dumps, create isolated SQLite databases, and keep each session’s artifacts under `backend/databases/`.
- **Schema explorer & visualizer** – Inspect tables, columns, foreign keys, and sample rows directly inside the workspace rails or the dedicated full-screen visualizer.
- **Conversational querying** – Ask natural-language questions in the chat pane; the backend generates, validates, and runs SQL, then streams back rows plus readable explanations.
- **Rich result views** – View tabular outputs, autogenerated charts, executed SQL, execution timing, and model confidence inside `ResultCard`.
- **Session history & management** – Switch between previously uploaded datasets, delete sessions, and resume context thanks to TanStack Query + `localStorage`.
- **Demo & landing experience** – `/demo` showcases the pipeline for new users, while the landing page highlights product value before authentication.
- **Authentication & permissions** – Clerk protects all `/api/...` routes; the frontend stores Clerk JWTs and the backend uses `req.userId` for authorization.
- **Embeddings & AI context** – Schema snippets are embedded (see `services/embeddings.js`) so `QueryGenerator` can assemble accurate SQL with relevant context.
- **Validator & safety rails** – `sqlValidator` enforces read-only queries and dialect rules before any statement runs against the session DB.
- **Extensible data adapters** – Managers exist for SQLite (default) plus stubs for MySQL/Postgres, making live connection support easier to add next.
...
2. **Environment variables**
   ...
   - Frontend (`Frontend/.env`):
     ```
     VITE_API_BASE_URL=https://jellyfish-app-z7mbn.ondigitalocean.app
